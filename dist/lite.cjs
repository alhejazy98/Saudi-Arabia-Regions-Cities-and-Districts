 ...
 */
function findNearestPoint(point, items) {
    if (items.length === 0)
        return undefined;
    let nearest = items[0];
 ...
 */
function getItemsWithinRadius(point, items, radiusKm) {
...
class SpatialIndex {
     ...
     */
    query(coord, radiusCells = 1) {
        const [lat, lon] = coord;
        const results = [];
        const centerCell = this.getCellCoords(coord);
        for (let latCell = centerCell.lat - radiusCells; latCell <= centerCell.lat + radiusCells; latCell++) {
            for (let lonCell = centerCell.lon - radiusCells; lonCell <= centerCell.lon + radiusCells; lonCell++) {
 ...
 */
function findNearestCity(lat, lon, cities, maxRadiusKm) {
    const point = [lat, lon];
    // Use spatial index for faster lookup
    if (!citySpatialIndex) {
        citySpatialIndex = new SpatialIndex(1); // 1-degree grid
 ...
 * });
 */
function findCitiesInRadius(lat, lon, cities, radiusKm) {
    const point = [lat, lon];
    const results = [];
 ...
 * Lon: 34.5 to 56.0
 */
function isWithinSaudiArabia(lat, lon) {
 ...
 */
function searchRegions(query, options = {}) {
    const { searchInArabic = true, searchInEnglish = true, caseSensitive = false, exactMatch = false, } = options;
    ...
    return regions.filter((region) => {
        ...
        if (exactMatch) {
            return ((searchInArabic && nameAr === searchQuery) ||
                (searchInEnglish && nameEn === searchQuery));
        }
        return ((searchInArabic && nameAr.includes(searchQuery)) ||
 ...
 */
function searchDistricts(query, options = {}) {
    const { searchInArabic = true, searchInEnglish = true, caseSensitive = false, exactMatch = false, } = options;
    const searchQuery = caseSensitive ? query : query.toLowerCase();
 ...
 */
function getCapitalCity(regionId) {
 ...
 * @returns Region object or undefined
 */
function getRegionOfCity(cityId) {
 ...
 * @returns Region object or undefined
 */
function getRegionOfDistrict(districtId) {
 ...
 * @returns City object or undefined
 */
function getCityOfDistrict(districtId) {
    const district = getDistrictById(districtId);
    if (!district)
        return undefined;
 ...
 */
function getRegionsByPopulation(ascending = false) {
...
exports.findNearestCity = findNearestCity;
...
exports.getDistrictsByRegion = getDistrictsByRegion;
exports.getItemsWithinRadius = getItemsWithinRadius;
exports.getRegionByCode = getRegionByCode;
exports.getRegionById = getRegionById;
exports.getRegionOfCity = getRegionOfCity;
exports.getRegionOfDistrict = getRegionOfDistrict;
exports.getRegionsByPopulation = getRegionsByPopulation;
exports.getTotalPopulation = getTotalPopulation;
exports.isWithinSaudiArabia = isWithinSaudiArabia;
exports.resetSpatialCache = resetSpatialCache;
